...
class ExecutionContext extends EventEmitter {
  ...
  addCleanupListener(fn) {
    this.on('cleanup', fn);
    return () => {
      this.removeListener('cleanup', fn);
    };
  }
  ...
  trackVariablesChange(prop, vars, valueGenerator) {
    const updater = () => {
      ...
      if (utils._isCursor(nextValue)) {
        this.trackCursorChange(prop, nextValue, vars);
        prop.emitChange();
    ...
    const varTrackers = Object.keys(vars).map(k =>
  ...
  trackCursorChange(prop, cursor, vars) {
    ...
    const observer = (result) => {
      if (Array.isArray(result)) {
        result = result.map(x => utils._createPropertyWithContext(x, vars))
      }
      prop(result);
    };
