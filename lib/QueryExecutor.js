...
export default class QueryExecutor extends EventEmitter {
  constructor(fragments, initVarsOverride, containerClass, prepareVariables) {
    super();
    this.containerClass = containerClass;
    this.fragmentNames = _keys(fragments);
    this.initVarsOverride = initVarsOverride;
    this.context = new ExecutionContext();
    this.variables = this.context.getVariables(containerClass,
      initVarsOverride, {}, prepareVariables);
  ...
  updateVariables(nextProps) {
    ...
    return this._execution.then(() => {
      ...
      _each(nextProps, (prop, k) => {
        if (this.variables[k] && this.variables[k]() !== prop) {
